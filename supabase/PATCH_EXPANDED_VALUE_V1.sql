-- EVE Football Scanner — EXPANDED VALUE PATCH V1
-- Run ONCE in the dedicated FREE eve-football-scanner Supabase project.
-- Adds bookmaker price/value fields to Market Lab without changing the calibrated model.
...
comment on view public.scanner_expanded_markets is
'Calibrated expanded-market research signals with conservative fair probability, latest compatible bookmaker price, edge, EV and value status. 2026/27 remains out-of-sample validation; VALUE does not promote a market to core Best Bets automatically.';

commit;

select 'EVE Expanded Value V1 patch complete' as status;
