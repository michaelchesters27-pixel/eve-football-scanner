-- EVE Football Scanner - VALUE ENGINE one-shot setup
-- Run this in the dedicated FREE eve-football-scanner Supabase project.
...
order by
  case when bo.decimal_odds is not null and p.fair_probability is not null then (p.fair_probability * bo.decimal_odds - 1) else -1 end desc,
  p.confidence desc;
...
select 'EVE Value Engine database setup complete' as status;
