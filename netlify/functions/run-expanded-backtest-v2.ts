...
const JOB = 'backtest-2526-v1-expanded-v2'
const MODEL = 'v1-expanded-research-v2-walk-forward'
...
function jointCandidate(
  ...
  // The v2 correction is deliberately selective. The 2025/26 read-only audit
  // showed that removing duplicated opposition evidence improved half-goals and
  // match-cards, and was non-worse for match-corners. BTTS did not improve, so
  // it remains on its validated v1 formula.
  const corrected = market === 'half_goals' || market === 'match_cards' || market === 'match_corners'
  const evidence: Evidence[] = [
    { key: 'recent', score: Math.round(recentRate) },
    { key: 'venue', score: Math.round(venueRate) },
    ...(!corrected ? [{ key: 'opponent', score: Math.round(recentRate) }] : []),
  ...
  const qualityParts: Array<[number, number, number]> = [
  ...
  if (market === 'match_cards') qualityParts.push([ref?.sample ?? 0, 10, 12])
  else if (!corrected) qualityParts.push([10, 10, 12]) // preserve validated v1 BTTS DQ exactly

  const quality = sampleQuality(qualityParts)
...
export default async () => {
  ...
  try {
    ...
    const bySelection = [...new Set(evaluated.map((x) => x.selectionKey))].map((selectionKey) => {
      const selectionRows = evaluated.filter((x) => x.selectionKey === selectionKey && x.dataQuality >= 70)
      return { selectionKey, ...stat(selectionRows) }
    })
    ...
    const summary = {
      ...
      methodology: 'Strict walk-forward. V2 selectively removes duplicated opposition evidence only in half-goals, match-cards and match-corners because those corrections passed the independent 2025/26 comparison. BTTS and team-goals remain on their validated v1 formula. Historical starting XI is unavailable, so lineup contribution is held neutral and receives no data-quality credit. Referee card history uses prior appointments only.',
      targetFixtures,
      skippedLowHistory,
      ...
      bySelection,
      calibrationBands,
      strongestLeagueMarkets: byLeague.slice(0, 40),
      correctionPolicy: {
        unchanged: ['btts','team_goals'],
        corrected: ['half_goals','match_cards','match_corners'],
        seasonLabel: 'Trailing 30 matches (legacy field key season retained for schema compatibility)',
      },
      caveat: 'Hit-rate calibration only. Betting profitability still requires bookmaker odds and out-of-sample validation.',
    }
