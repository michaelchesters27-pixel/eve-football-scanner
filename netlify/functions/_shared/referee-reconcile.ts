...
export async function reconcileFixtureReferee(supabase:Supabase,fixtureId:string){
  const index=await buildIndex(supabase,true)
  ...
  return reconcileWithIndex(supabase,fixture,String(context?.referee_name??'').trim(),index)
}
...
export async function reconcileActiveReferees(supabase:Supabase){
  ...
  const {data:fixtures,error}=await supabase.from('fixtures').select('id,referee_id').in('status',['scheduled','live']).gte('kickoff',from).lte('kickoff',to).order('kickoff',{ascending:true}).limit(32)
  ...
  for(const fixture of fixtures??[]){
    ...
    try{ results.push(await reconcileWithIndex(supabase,fixture,officialName,index)) }
